...
## 13. Self-updating from Telegram (§13.5 layout implemented 2026-09-07; §13.2/§13.3 gateway-side pieces still planned)
...
### 13.2 Version-check + notification (decided)

- A poller in `apps/gateway`, same shape as the existing ban/IP-ban expiry poller (§5.7), checks
  `api.github.com/repos/constantant/cod2admin/releases/latest` on a fixed interval (default:
  every 6h — a version check doesn't need §5.7's ~10s cadence) and compares the tag against the
  running version (`package.json`, already shipped in the bundle per
  `scripts/build-installer-bundle.sh`).
- On a new tag, DMs the **owner only** (not all admins — updating is an owner-level decision,
  matching who can already run `/rcon`, §6) with the version and the relevant `CHANGELOG.md`
  section for that release (already generated by `nx release`, see `CLAUDE.md`'s Versioning
  section — no new changelog machinery needed, just slicing the existing file).

### 13.3 `/update` command (decided)

Owner-only, same permission tier as `/rcon`.

1. Re-checks latest release, replies with a confirmation prompt (inline `Update` / `Cancel`) —
   this restarts the service, so it's never a one-shot no-confirm command.
2. On confirm, downloads that release's asset — the same single `cod2admin-<version>.tar.gz` §12
   already publishes, no second release asset needed — into a directory the `cod2admin` user can
   write, e.g. `/opt/cod2admin/staging` (created by `install.sh`).
3. Verifies a checksum before trusting anything in it — `scripts/build-installer-bundle.sh`/
   `scripts/release.sh` need to start publishing a `.sha256` alongside the tarball (not done
   today).
4. Writes a small "pending update" marker into `staging/` (chat id + target version).
5. Runs `sudo /opt/cod2admin/bin/apply-update.sh <staged-tarball>` — the one command the sudoers
   rule (§13.4) permits — and replies "Updating, back in a few seconds."
6. The gateway process itself may go down mid-step 5 as `apply-update.sh` restarts the service —
   expected. The **new** process checks for the pending-update marker on boot, posts "✅ Updated
   to vX.Y.Z" to the same chat, and deletes the marker.
